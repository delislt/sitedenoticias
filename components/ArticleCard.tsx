...
export function ArticleCard({ article }: { article: Article }) {
  const postedAt = formatDate(article.published_at || article.created_at);
      ...
      <Link href={"/artigo/" + article.slug} className="block cursor-pointer">
        <div className="relative aspect-video w-full overflow-hidden bg-zinc-800">
          {article.coverImage ? (
            <Image
              ...
              quality={75}
              unoptimized={article.coverImage.startsWith("/api/media/")}
        ...
        <div className="space-y-3 p-5">
          ...
          <p className="text-xs text-gold">
            {kindLabels[article.kind || "simulation"]}
          </p>
          <h3 className="font-display text-2xl leading-tight text-zinc-100 group-hover:text-gold">
            {article.title}
          </h3>
