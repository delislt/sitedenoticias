...
export default async function SearchPage({
  ...
  if (query) {
    ...
    const { data, error } = await supabase
      ...
      .select('*')
      .or(
        `title.ilike.%${query}%,subtitle.ilike.%${query}%,author.ilike.%${query}%,content.ilike.%${query}%`
      )
    ...
    <div className="container-premium space-y-8 py-10">
      ...
      <form className="card-border flex flex-col gap-3 p-4 md:flex-row" action="/busca">
        <input
          ...
          defaultValue={q ?? ''}
        />
        <button className="bg-gold px-6 py-3 font-semibold text-ink transition hover:bg-amber-400">
          Buscar
        </button>
      </form>
