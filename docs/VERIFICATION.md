...
- `node --test tests/database.test.mjs tests/email-otp.test.mjs`: 21 testes aprovados (duas suítes e 19 cenários).
...
Os testes isolados cobrem também a separação entre senha/Google e código por email, vínculo do código à sessão primária, expiração, limite de cinco tentativas, não reutilização, troca de email, revogação de sessão, foto privada e bloqueio da gestão de edições.
...
| Conta                      | Cadastro, Google, senha, confirmação, recuperação, troca de senha, nome e foto implementados; telas públicas e retorno pós-confirmação conferidos no navegador |
...
Três migrations aplicadas pelo conector Supabase. A terceira acrescentou o código por email vinculado à sessão, bucket privado de fotos e retirou a gestão de múltiplas edições. O cadastro foi aberto; comentários permanecem fechados.
...
3. Cadastro, email/senha e Google estão ativos; a Site URL foi corrigida para produção. O Supabase ainda usa o serviço provisório de email, limitado a duas mensagens por hora. Por isso, o segundo código está disponível, mas `email_otp_required` permanece falso até conectar SMTP de produção.
